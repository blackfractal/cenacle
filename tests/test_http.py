...
class HTTPTests(unittest.TestCase):
    ...
    def test_full_message_search_and_notes_separate(self):
        ...
        m = self.cmd("send", {"body": "needle " + "X"*20000}, a["credential"])
        _, _, body = self.request("/api/history?project="+self.p.id+"&room=agent_chat&q=needle")
        self.assertEqual(1, len(json.loads(body)["messages"]))
        _, _, body = self.request("/api/history?project="+self.p.id+"&room=agent_chat")
        self.assertEqual(1, len(json.loads(body)["messages"]))
