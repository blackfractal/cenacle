...
from vibeguild.folder_picker import choose_folder, PickerError
...
class FolderPickerTests(unittest.TestCase):
    def test_nonexistent_coordination_path_survives_native_json_roundtrip(self):
        with tempfile.TemporaryDirectory() as temporary:
            target = Path(temporary) / "Clé with spaces" / ".vibeguild"
            output = CompletedProcess([], 0, json.dumps({"path": str(target)}), "")
            with patch("vibeguild.folder_picker.subprocess.run", return_value=output) as child:
            ...
            with patch("vibeguild.folder_picker.subprocess.run", return_value=output):
